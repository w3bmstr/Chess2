...
searchBox.addEventListener('input', function() {
  ...
  const appendResultsMessage = (text) => {
    ...
    // Mobile Moves/Openings view: hide the older PGN search UI to avoid clutter.
    // (Search remains available under the drawer 'Search' tab.)
    try {
  ...
  if (isMobileNav) {
    ...
    if (anchor) {
      ...
      if (searchBox) moveList.insertBefore(searchBox, anchor);
      if (resultsDiv) moveList.insertBefore(resultsDiv, anchor);
    } else {
      moveList.appendChild(btnRow);
      moveList.appendChild(explorer);
      if (searchBox) moveList.appendChild(searchBox);
      if (resultsDiv) moveList.appendChild(resultsDiv);
    }
  } else {
    moveList.appendChild(btnRow);
    moveList.appendChild(explorer);
